...
while True:
    ...
    if imagen_A6 is not None:
        ...
        for c_2 in contorno2:
            ...
            if (Momentos["m00"] == 0):  # Agregar etiquetas en objetos
                Momentos["m00"] = 1.0
            x = int(Momentos["m10"]/Momentos["m00"])
            y = int(Momentos["m01"]/Momentos["m00"])
            if area < 16469 and area > 12000:
                font = cv2.FONT_HERSHEY_SIMPLEX
                cv2.putText(imagen_A6, "$10", (x, y),
                           font, 0.75, (0, 255, 0), 2)
                suma1 = suma1+10
            if area < 11971 and area > 10000:
                font = cv2.FONT_HERSHEY_SIMPLEX
                cv2.putText(imagen_A6, "$5", (x, y),
                           font, 0.75, (0, 255, 0), 2)
                suma2 = suma2+5
            if area < 9700 and area > 8000:
                font = cv2.FONT_HERSHEY_SIMPLEX
                cv2.putText(imagen_A6, "$10", (x, y),
                           font, 0.75, (0, 255, 0), 2)
                suma3 = suma3+2
